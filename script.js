const pcItems = [
  {
    ...
    description: "Ideální sestava pro hraní ve Full HD, běžný streaming i náročnější domácí použití.",
...
const affiliateItems = [
  {
    ...
    price: "od 1 690 Kč",
    text: "Skvělý upgrade pro starší notebook nebo PC. Výrazně zrychlí systém i běžnou práci.",
  ...
  },
  {
    ...
    store: "Doporučený nákup",
    price: "od 1 290 Kč",
    text: "Rozumná volba pro kancelářské i domácí počítače, kde chybí výkon při více spuštěných aplikacích.",
    linkText: "Zobrazit doporučení"
  },
  {
    ...
    store: "Doporučený nákup",
    price: "od 2 490 Kč",
    text: "Praktické tipy na kancelářské i univerzální monitory s dobrým poměrem cena / výkon.",
    linkText: "Zobrazit doporučení"
  }
];
...
function renderPcCards(items) {
  ...
  pcGrid.innerHTML = items.map((item) => `
...
function renderAffiliateCards() {
  ...
  affiliateGrid.innerHTML = affiliateItems.map((item) => `
...
filterButtons.forEach((button) => {
  button.addEventListener("click", () => {
    ...
    renderPcCards(pcItems.filter((item) => item.category === filter));
  });
});
